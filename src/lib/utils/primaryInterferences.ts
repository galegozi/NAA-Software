 ...
 * fast-neutron threshold reaction. Used to suggest corrections in Step 1; it
 * lists relationships only — the factor depends on the irradiation position's
 * fast/thermal flux ratio and must be measured for the facility. Not exhaustive:
 * the wizard also accepts any other interfering element.
 *
...
export const PRIMARY_INTERFERENCES: readonly PrimaryInterference[] = [
	...
	{ product: 'Mg-27', interferingElement: 'Si', reaction: 'Si-30(n,α)Mg-27' },
...
/**
 * Identity key for an analysis isotope. Accepts "Mg-27" in the name, or a bare
 * mass number ("27") with the element in `elementName`. Falls back to the raw
 * name so two unparseable entries still compare sensibly.
 */
export function analysisIsotopeKey(
	isotope: Pick<IsotopeInfo, 'elementName' | 'isotopeName'>
): string {
	const name = (isotope.isotopeName ?? '').trim();
	const parsed =
		parseIsotopeName(name) ??
		(/^\d{1,3}\s*[A-Za-z0-9]*$/.test(name)
			? parseIsotopeName(`${lookupElementSymbol(isotope.elementName ?? '')}-${name}`)
			: null);
	return parsed ? isotopeIdentityKey(parsed) : `raw:${name.toLowerCase()}`;
}

/** Element symbol of an analysis isotope ("" when it can't be worked out). */
export function analysisIsotopeElement(
	isotope: Pick<IsotopeInfo, 'elementName' | 'isotopeName'>
): string {
	const fromLabel = lookupElementSymbol(isotope.elementName ?? '');
	if (fromLabel) return fromLabel;
	const parsed = parseIsotopeName(isotope.isotopeName ?? '');
	return parsed ? lookupElementSymbol(parsed.shortName) : '';
}

const BY_PRODUCT = new Map<string, PrimaryInterference[]>();
for (const entry of PRIMARY_INTERFERENCES) {
	const key = analysisIsotopeKey({ elementName: '', isotopeName: entry.product });
...
export function primaryInterferencesFor(
	...
	return BY_PRODUCT.get(analysisIsotopeKey(isotope)) ?? [];
}
