<script lang="ts">
	...
	let signedInName = $derived(
		swaAuth.principal?.userDetails || swaAuth.principal?.userId || 'your account'
	);
</script>
...
<div class="app-shell">
	<nav class="top-nav" aria-label="Primary">
		<button
			type="button"
			class="top-nav__wordmark"
			onclick={() => analysisMeta.goToWelcome()}
			...
			{#if swaAuth.signInAvailable}
				{#if swaAuth.signedIn}
					<span class="top-nav__auth">
						...
						{#if !swaAuth.hasRole(WRITER_ROLE)}
							<span class="top-nav__auth-note">(no upload access)</span>
						{/if}
						<span aria-hidden="true">·</span>
						<button type="button" class="top-nav__auth-action" onclick={() => signOut()}>
							Sign out
						</button>
					</span>
				{:else}
					<button
						type="button"
						class="top-nav__auth-action"
						onclick={() => redirectToSignIn()}
						disabled={swaAuth.checking}
					>
						{swaAuth.checking ? 'Checking sign-in…' : 'Sign in for uploads'}
					</button>
				{/if}
			{/if}
